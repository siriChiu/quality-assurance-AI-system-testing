...
from qa_aist.config import default_config, load_project_config, validate_config_data
...
class ConfigTest(unittest.TestCase):
    ...
    def test_schema_rejects_raw_secret(self) -> None:
        data = {
            ...
            "paths": {key: f".qa-aist-project/{key}" for key in ["workspace", "cases", "runners", "rules", "state", "evidence", "reports"]},
    ...
    def test_load_generated_config(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / ".qa-aist.yaml").write_text(default_config(), encoding="utf-8")
            config = load_project_config(root)
            self.assertEqual(config.data["project"]["name"], "example-project")
            self.assertEqual(config.paths.cases, root / ".qa-aist-project" / "cases")
