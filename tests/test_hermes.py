...
class HermesDispatchTest(unittest.TestCase):
    ...
    def test_install_skill_creates_hermes_skill_slash_command(self) -> None:
        with tempfile.TemporaryDirectory() as skills_tmp:
            ...
            self.assertIn("required_companion_commands:", frontmatter)
            self.assertIn("- cases generate", frontmatter)
            self.assertIn("- issues create-from-failure", frontmatter)
            self.assertIn("- close-loop", frontmatter)
            ...
            self.assertIn("Gitea MCP failed-case issue creation workflow", text)
            self.assertIn("/quality-pilot issues create-from-failure --remote --all", text)
            self.assertIn("/quality-pilot issues create-from-failure --local --all", text)
            ...
            self.assertIn("MANDATORY SYSTEM GATE — grill-me for production commands", text)
            ...
            self.assertIn("stratified first slice", text)
            self.assertIn("exit-code-only surface probe is partial evidence", text)
            self.assertIn("Zero-case, dry-run, and partial-only scopes cannot become an official PASS", text)
            self.assertIn("one deterministic tick", text)
            self.assertIn("--fail-on-test-failure", text)
            self.assertIn("Do not claim deep white-box", text)
