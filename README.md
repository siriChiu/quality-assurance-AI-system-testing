...
flowchart TD
  ...
  B0 --> C["cases validate<br/>qa-test dry-run"]
  B --> C
  ...
  W1 --> F{"Wiki gate"}
  F -->|HTTP token| G["update Wiki page"]
  F -->|MCP backend| M["gated MCP write request"]
  M --> N["Hermes Gitea MCP<br/>complete-mcp"]
...
接著 Hermes 只需要在你確認後用 Gitea MCP 讀 issues，寫入 `mcp_issues_json`，再執行 `/qa-aist issues sync`。同一個 MCP backend 也可在 Wiki gate 通過後，由 QA-AIST 產生 gated Wiki write request，讓 Hermes 用 Gitea MCP 更新指定 Wiki 頁。
...
如果你的 Hermes 已經有 Gitea MCP，QA-AIST 支援 MCP issue sync 與 gated Wiki handoff：Hermes 先用 Gitea MCP 讀 issues，將原始 JSON 寫入 `mcp_issues_json`，再呼叫 `/qa-aist issues sync`；Wiki 更新則必須先跑 `/qa-aist publish wiki apply`，收到 `needs_mcp_apply` 後，Hermes 只能依 `mcp_write_request` 更新同一個 Wiki page，再用 `complete-mcp` 回填結果。這條路不需要 token，但 issue comment、issue create、PR 仍不支援 MCP 寫入。
...
`/qa-aist cases generate` 不會默默猜模式。首次導入請用 `/qa-aist cases generate --init`，它會像有主見的 SWQA 工程師一樣分析 README、程式碼、package metadata、既有 runners/cases/rules，建立「基本都要測」且可執行的初始測試地圖。每個 `INIT-*` case 都會先綁定 side-effect-safe command，例如 CLI help/parser、repo compile/build probe、`go run ./cmd/<tool> --help`、invalid command rejection 或 bounded loop baseline；缺 lab target、fixture、帳密時只列為 advisory，不會讓 case 變成 placeholder。後續有新 issues、PR、latest run 或 reports 時，才用 `/qa-aist cases generate --growing`，像蜘蛛網一樣從最新狀態擴散新的測試假說。若 payload 回傳 `hermes_needs_input`，Hermes 必須呼叫 `clarify` 補齊少量大分類阻擋資訊；不要一個一個 testcase 討論。

`--init` 預設不套任意數量上限；它會依 repo 掃描到的 surfaces 與固定 SWQA 維度產生完整初始地圖。`--generated_count <max>` 只用在你刻意想縮小第一批 exploratory cases 時，例如 `/qa-aist cases generate --init --generated_count 5`。若你想大幅減少互動，使用 `/qa-aist cases generate --init --fast`，QA-AIST 會以最高安全標準自行決策：repo-only、read-only、dry-run、parser-only、mock/no-op fixture 優先。高副作用或 lab-only 測試不會阻塞 init case；會先生成可跑的 safe probe，後續再加強成 lab runner。

## Guided Interaction

QA-AIST 的 Hermes skill 不是被動轉貼 JSON。每次 `/qa-aist ...` 執行後，engine 會回傳 `next_actions`，Hermes 應該用繁體中文列出下一步選單，讓使用者回覆編號或直接輸入下一個指令。

如果 engine 回傳 `payload.input_required: true`、`payload.interaction.type: "needs_input"` 或 `payload.hermes_needs_input.status: "required"`，Hermes 應呼叫 `clarify`。`payload.hermes_needs_input.questions[]` 是唯一標準題目來源，內容應是大分類阻擋資訊，不是逐案 testcase 討論；`clarify` 不可用時，才在聊天室列出題目並等待使用者回答。
...
- 會寫檔、跑測試、讀 Gitea MCP、用 Gitea MCP 更新 Wiki、publish、push branch、建立 PR 的動作，Hermes 必須先問使用者確認。
- `cases generate --init` 或 `cases generate --growing` 預設產出可執行 safe probes，不要逐案確認 testcase；只有 engine 明確回傳 `hermes_needs_input` 時，Hermes 才問少量大分類阻擋資訊。
- `cases generate --init/--growing`、`qa-test run-one/run`、`close-loop run-once` 後，QA-AIST 會自動產生 Wiki 狀態 plan；HTTP backend 會在 token/gate 通過後直接更新 Wiki，MCP backend 會產生 gated `mcp_write_request` 等 Hermes 執行。
...
| 首次建立 SWQA cases | `/qa-aist cases generate --init` | 分析 README/code/metadata/runners/rules，產生可執行的功能、正向、反向、邊界、壓力 safe-probe contracts |
| 依最新狀態擴散 cases | `/qa-aist cases generate --growing` | 依 issues/PR/latest-run/reports/cases/runners 長出 incremental executable contracts |
| 指定功能產生初始 cases | `/qa-aist cases generate --init --feature "CLI help" --profile cli` | 讓 QA-AIST 針對功能/profile 產生完整初始建案；`--generated_count <max>` 只在你想手動縮小範圍時使用 |
| 快速自主初始建案 | `/qa-aist cases generate --init --fast` | 以最高安全標準自行決策，減少互動問答，不逐一確認 testcase |
| 只產生第一批 N 個 | `/qa-aist cases generate --init --generated_count 5` | 明確限制 generated draft 數量 |
| 匯入 growth session 候選 | `/qa-aist cases generate --growing --candidate-json <path>` | Hermes 獨立 session 只產候選 JSON，engine 驗證後才寫 case |
| 查看待補強 drafts | `/qa-aist cases review` | 只在 engine 回報 needs-input 或人工想補 lab runner 時使用 |
...
| 手動更新 Wiki | `/qa-aist publish wiki apply` | HTTP backend 直接更新 Wiki；MCP backend 產生 gated write request |
| 完成 MCP Wiki 寫入 | `/qa-aist publish wiki complete-mcp --result-json <path>` | Hermes 用 Gitea MCP 寫完 Wiki 後回填結果 |
...
`/qa-aist cases generate --init` 會用這個 policy pack 建立完整初始 executable cases，預設覆蓋功能、正向、反向、邊界、壓力/timeout 與 side-effect-safe 風險。`/qa-aist cases generate --growing` 則在後續依最新 issues/PR/run/report 狀態擴散：
...
`cases generate --init` 會產生 `source.type: init` 的初始 executable contracts，固定涵蓋 functional、positive、negative、boundary、invalid input、side-effect-safe、stress/timeout-risk 等 SWQA 維度。`cases generate --growing` 會產生 `source.type: growth` 的增量 executable contracts，附 `growth_seed`、`six_hats` 與 `growth_reason`。Hermes 不需要逐案確認；QA-AIST 會先用 side-effect-safe probe 讓每個 case 能跑，後續才由使用者按大分類加上 lab runner、fixture、輸入檔、credential env、成功條件與副作用邊界。
...
|---|---|---|
| `cases generate --init/--growing` | `case_generation` | 更新 generated case、advisory input、六色帽摘要；可沒有 test evidence，但不宣稱 PASS/FAIL |
...
自動寫入規則很嚴格：只有 `tracker.provider: gitea` 且 Wiki gate 通過時，QA-AIST 才會進入遠端 Wiki 流程。HTTP backend 會在 token env 存在時直接更新遠端 Wiki；MCP backend 會寫出 gated request，交給 Hermes Gitea MCP 執行，再用 `complete-mcp` 回填。若 tracker disabled、token missing、gate blocked 或 Hermes 尚未完成 MCP 寫入，只會寫本地：
...
| `tracker.gitea.backend: mcp` issue/comment/PR write | blocked |
| `tracker.gitea.backend: mcp` Wiki write without gated request | blocked |
...
`publish wiki apply` 只更新 Wiki，不會建立 issue comment、new issue 或 PR。MCP backend 時，Hermes 也只能更新 QA-AIST request 指定的同一個 Wiki page。
...
Usually because Wiki gate blocked it, Gitea HTTP token env is missing, evidence is missing for a test-result update, tracker provider is disabled, or MCP backend still needs Hermes to complete the gated Wiki handoff.
...
If `.qa-aist.yaml` uses `tracker.gitea.backend: mcp`, `publish apply` and `fix-issues submit-pr` are intentionally blocked. MCP backend supports issue sync plus gated Wiki-only handoff. Use `backend: http` plus `QA_AIST_GITEA_TOKEN` for issue comments, issue creation, or PR writes.
...
Yes. Wiki can be updated automatically or through `publish wiki apply` after the Wiki gate passes. HTTP backend writes Wiki directly with `QA_AIST_GITEA_TOKEN`; MCP backend returns `needs_mcp_apply`, then Hermes updates the configured Wiki page through Gitea MCP and records the result with `complete-mcp`. Issue comments/issues require `publish apply`; PRs require `fix-issues submit-pr`, and those still require HTTP token mode.
