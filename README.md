...
- `cases generate --init` 或 `cases generate --growing` 若產生 `hermes_needs_input`，Hermes 只問大分類阻擋資訊，不要逐案確認 testcase；`review_required_before_run` 的 draft 會被 engine 擋成 `BLOCK`，不會直接執行。
...
`/qa-aist cases generate --init` 會用這個 policy pack 建立完整初始 draft cases，預設覆蓋功能、正向、反向、邊界、壓力/timeout 與 side-effect-safe 風險。`/qa-aist cases generate --growing` 則在後續依最新 issues/PR/run/report 狀態擴散：
...
`cases generate --init` 會產生 `source.type: init` 的初始 draft，固定涵蓋 functional、positive、negative、boundary、invalid input、side-effect-safe、stress/timeout-risk 等 SWQA 維度。`cases generate --growing` 會產生 `source.type: growth` 的增量 draft，附 `growth_seed`、`six_hats` 與 `growth_reason`。Hermes 只需依大分類補齊測試需要的 runner、fixture、輸入檔、環境變數、成功條件、side-effect safe 邊界，再跑正式測試。若 draft 標示 `review_required_before_run: true`，正式 `qa-test run` 會回 `BLOCK`。
