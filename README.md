...
`/qa-aist cases generate` 不會默默猜模式。首次導入請用 `/qa-aist cases generate --init`，它會分析 README、程式碼、package metadata、既有 runners/cases/rules，從專業 SWQA 角度產生功能、正向、反向、邊界、壓力/timeout 等 draft cases。後續有新 issues、PR、latest run 或 reports 時，才用 `/qa-aist cases generate --growing`，像蜘蛛網一樣從最新狀態擴散新的測試假說。若回傳 questions，Hermes 必須先用繁體中文問答補齊測試輸入、fixture、成功條件與副作用邊界，再把 draft case 當成可執行測試。
...
- `cases generate --init` 或 `cases generate --growing` 若產生問題，Hermes 要逐題用繁體中文問答補齊，不要亂猜；`review_required_before_run` 的 draft 會被 engine 擋成 `BLOCK`，不會直接執行。
...
| 指定功能產生初始 cases | `/qa-aist cases generate --init --feature "CLI help" --profile cli --count 5` | 讓 QA-AIST 針對功能/profile 引導初始建案 |
...
PYTHONPATH=src python3 -m qa_aist.cli cases generate --root /path/to/product --init --feature "CLI help" --profile cli --count 5
