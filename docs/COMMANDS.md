...
它會列出 blocker/warning，例如 Redmine generic probe、evidence-contract mismatch、stale MCP issue-write request、active issue without runnable case、Wiki READY/NOT_RUN disagreement、missing Hermes MCP status、subagent profile incomplete。`doctor` 和 `issues status` 也會帶出 state audit 摘要。
