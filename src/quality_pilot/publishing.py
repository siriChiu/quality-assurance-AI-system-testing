...
def plan_publish(config: ProjectConfig, *, latest_run: str | Path | None = None) -> dict[str, Any]:
    ...
    actions.append(
        {
            ...
            "message": "AI Quality Pilot test status update",
            "body": wiki_body,
        ...
        actions.append(
            {
                ...
                "issue_id": issue_id,
                "title": f"[QA][AUTO][{result.get('case_id')}] {result.get('title') or 'AI Quality Pilot failure'}",
                "body": body,
    ...
    plan = {
        "schema": "quality-pilot.publish-plan.v1",
...
def apply_publish_plan(config: ProjectConfig, *, plan_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if gitea_cfg.uses_mcp:
        return {
            ...
            "message": "tracker.gitea.backend: mcp supports issue sync and gated Wiki-only handoff. Configure HTTP backend with token_env before legacy /quality-pilot publish apply writes issue comments or mixed publish output.",
    ...
    for action in plan.get("actions", []):
        ...
        if kind == "wiki_update":
            response = client.update_wiki_page(page=str(action.get("page") or gitea_cfg.wiki_page), content=str(action.get("body") or ""), message=str(action.get("message") or "AI Quality Pilot update"))
            ...
            response = client.create_issue(title=str(action.get("title") or "AI Quality Pilot failure"), body=str(action.get("body") or ""))
        else:
...
def render_issue_body(result: dict[str, Any], source: dict[str, Any]) -> str:
    ...
    lines = [
        "## AI Quality Pilot failure report",
        "",
    ...
    lines.extend(["", "## Commands", "", *command_lines, "", "## Notes", "", "Generated by AI Quality Pilot publish plan after deterministic test execution."])
    return "\n".join(lines) + "\n"
