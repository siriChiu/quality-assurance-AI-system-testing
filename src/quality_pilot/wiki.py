...
def plan_wiki(
    ...
    *,
    ...
    plan = {
        ...
        "next_action": "/quality-pilot publish wiki apply" if status == "ready" else "/quality-pilot publish wiki status",
        "created_at": utc_now(),
    }
...
def apply_wiki_plan(config: ProjectConfig, *, plan_path: str | Path | None = None) -> dict[str, Any]:
    ...
    try:
        response = GiteaClient(gitea_cfg).update_wiki_page(
            ...
            message=str(plan.get("message") or "AI Quality Pilot wiki status update"),
        )
    except GiteaError:
        raise
    payload = {
    ...
    *,
    ...
    if not _auto_wiki_enabled(config):
        return {
            ...
            "page": gitea_config_from_project(config.data).wiki_page,
            "next_action": "/quality-pilot publish wiki plan",
        }
    try:
        plan = plan_wiki(
            config,
...
def render_wiki_body(
    *,
    ...
    lines.extend(["", "_Generated by AI Quality Pilot deterministic Wiki renderer._", ""])
    return "\n".join(lines)
...
def _wiki_commit_message(event: str) -> str:
    return {
        "manual": "AI Quality Pilot wiki status update",
        "case_generation": "AI Quality Pilot draft case status update",
        "test_result": "AI Quality Pilot test result status update",
        "gitea_write_summary": "AI Quality Pilot Gitea write summary update",
    }[event]
...
def _build_mcp_wiki_request(config: ProjectConfig, *, plan: dict[str, Any], remote: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "hermes_instructions": [
            ...
            "Write the MCP tool result JSON to `result_path`, then report the result as part of the same `/quality-pilot publish wiki apply` user flow.",
        ],
        "created_at": utc_now(),
    }
